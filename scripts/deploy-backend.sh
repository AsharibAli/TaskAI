...
# Backend Deployment Script for DigitalOcean Kubernetes
# Usage:
#   ./deploy-backend.sh          # Deploy with :latest tag
#   ./deploy-backend.sh v1.0.1   # Deploy with specific version
...
IMAGE_TAG="${REGISTRY}/backend:${VERSION}"

echo ""
echo "🚀 Deploying Backend to Kubernetes"
echo "=================================="
echo "Registry: ${REGISTRY}"
echo "Image: backend:${VERSION}"
...
echo "⏳ Waiting for rollout to complete (3 replicas)..."
...
kubectl get deployment ${DEPLOYMENT} -n ${NAMESPACE} -o jsonpath='{.spec.template.spec.containers[?(@.name=="backend")].image}{"\n"}'
echo ""
...
if [ "$DAPR_ENABLED" = "true" ]; then
    echo "✅ Dapr sidecar enabled (app-id: backend)"
    echo "   All pods should show 2/2 READY (app + daprd)"
else
    echo "⚠️  Dapr sidecar disabled"
fi
echo ""

echo "🎉 Backend deployment complete!"
echo ""
echo "🌐 Backend API: https://taskai.asharib.xyz/api"
echo "📋 Health check: https://taskai.asharib.xyz/health"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    echo ""
    echo "📋 Recent backend logs (Ctrl+C to exit):"
    kubectl logs -n ${NAMESPACE} -l app.kubernetes.io/name=backend -c backend --tail=50 -f
fi
